<?php
	...
	if (!function_exists('__time')) {
		function __time($val)
		{
			...
			return date_i18n(get_option('time_format'), $val);
		}
	}

	if (!function_exists('__datetime')) {
		function __datetime($val)
		{
			if(!is_numeric($val))
			{
				$val = strtotime($val);
			}
			return date_i18n(get_option('date_format') . ' ' . get_option('time_format'), $val);
		}
	}
	...
	if (!function_exists('kei_post_isChecked')) {
		function kei_post_isChecked($val)
		{
			if(isset($_POST[$val])) :
				return 1;
			endif;
